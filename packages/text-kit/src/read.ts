...
export const core = (text: string): TextKitCore => {
  ...
  const eol = (ln: number): SourcePoint | undefined => {
    ...
    return (text.charAt(endIndex).match(/$/gm) ?? []).length > 1 ? end : eof()
  }
  ...
  const fixPoint = ({ line, column }: Point): SourcePoint => {
    ...
    const maxCol =
      (line < lines.length ? lines[line] : text.length + 1) -
      targetLineStartIndex
  ...
  const findLine = (index: number): number => {
    const l = lines.findIndex((_l, i) =>
      i === lines.length - 1 ? true : index < lines[i + 1]
    )
    return l === -1 ? 1 : l + 1
  }
  ...
  const substring = ({
    ...
    if (end === 'EOL') {
      const line = text.substring(
        startIndex,
        toIndex({ line: start.line, column: Infinity }) + 1
      )
      return line.split(/$/gm)[0]
    } else if (end === 'EOF') {
