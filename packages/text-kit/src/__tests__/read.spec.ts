...
const pos = ((
  [startLine, startColumn, offset1]:
    | [number, number]
    | [number, number, number],
  [endLine, endColumn, offset2]: [number, number] | [number, number, number]
): Position => ({
  start: point(startLine, startColumn, offset1),
  end: point(endLine, endColumn, offset2),
})) as {
  (
    [startLine, startColumn, offset1]: [number, number, number],
    [endLine, endColumn, offset2]: [number, number, number]
  ): SourcePosition
  (
    [startLine, startColumn]: [number, number],
    [endLine, endColumn]: [number, number]
  ): Position
}
...
type Params = {
  [K in keyof Omit<TextKit, 'numberOfLines'>]: Parameters<TextKit[K]>
}
type Return = {
  [K in keyof Omit<TextKit, 'numberOfLines'>]: ReturnType<TextKit[K]>
}
type SingleOrListParam<K extends keyof Omit<TextKit, 'numberOfLines'>> =
  Params[K] extends [any]
    ? Exclude<Params[K][0], Function | Array<any>>
    : Params[K]

const testReaderFn =
  <K extends keyof Omit<TextKit, 'numberOfLines'>>(pty: K) =>
  (
    testName: string,
    text: string,
    ...rest: Params[K] extends []
      ? [expected: Return[K] | ((r: TextKit) => Return[K])]
      : [
          testArgs:
            | SingleOrListParam<K>
            | ((r: TextKit) => SingleOrListParam<K>),
          expected: Return[K] | ((r: TextKit) => Return[K])
        ]
  ) => {
    const [testArgs, expected] =
      rest.length === 2 ? rest : [[] as SingleOrListParam<K>, rest[0]]
    return testReader(testName, text, (r) => {
      const calcArgs: SingleOrListParam<K> =
        typeof testArgs === 'function' ? testArgs(r) : testArgs
      const args: Params[K] =
        calcArgs instanceof Array ? calcArgs : ([calcArgs] as any)
...
describe('numberOfLines', () => {
  const testNumberOfLines = (
    testName: string,
    text: string,
    expected: number
  ) => {
    return testReader(testName, text, (r) =>
      expect(r.numberOfLines).toEqual(expected)
    )
  }
...
describe('substring', () => {
  ...
  describe('out-of-bounds', () => {
    testSubstring(
      'start line before beginning of document, end after',
      'test',
      pos([-1, 1], [2, 1]),
      'test'
    )
    testSubstring(
      'start line in document, end after',
      'test',
      pos([1, 1], [2, 1]),
      'test'
    )
    testSubstring(
      'start line before beginning of document, end within',
      'test',
      pos([-1, 1], [1, 2]),
      't'
    )
  ...
  testSubstring(
    'end is equal to start, inclusive end',
    'test',
    pos([1, 3], [1, 4]),
    's'
  )
  ...
  describe('end is EOL', () => {
    testSubstring(
      'with one line, no newline',
      'test',
      { start: point(1, 1), end: 'EOL' },
      'test'
    )
    testSubstring(
      "with one line, newline, doesn't include newline",
      'test\n',
      { start: point(1, 1), end: 'EOL' },
      'test'
    )
    testSubstring(
      'with one line, newline, inclusive end, includes newline',
      'test\n',
      { start: point(1, 1), end: 'EOL' },
      'test'
    )
  ...
  describe('end is EOF', () => {
    testSubstring(
      'with one line',
      'test',
      { start: point(1, 1), end: 'EOF' },
      'test'
    )
    testSubstring(
      'with multiple lines',
      'test1\ntest2\ntest3',
      { start: point(2, 1), end: 'EOF' },
      'test2\ntest3'
    )
    testSubstring(
      'with multiple lines and ending newline',
      'test1\ntest2\ntest3\n',
      { start: point(2, 1), end: 'EOF' },
      'test2\ntest3\n'
    )
...
describe('location', () => {
  ...
  describe('location out of range', () => {
    ...
    testLocation(
      'index higher than EOF, multiline',
      'test1\ntest2',
      12,
      point(2, 6, 11)
    )
  ...
  describe('location inverse of index', () => {
    for (let i = 0; i <= 11; i++) {
      testReader(`with i=${i}`, 'test1\ntest2', (r) =>
        expect(r.location(i).offset).toEqual(i)
      )
    }
  })
})
...
describe('match', () => {
  const testMatch = (
    ...
    expected:
      ...
      | ((r: TextKit) => [SourcePosition, string[]] | undefined)
  ) => {
  ...
  describe('multiline', () => {
    testMatch(
      ...
      '_Test\nTest_',
      [/[\s\S]*/m, pos([1, 1], [2, 5])],
      [pos([1, 1, 0], [2, 5, 10]), ['_Test\nTest']]
    )
    testMatch(
      'star without multiline flag',
      '_Test\nTest_',
      [/[\s\S]*/, pos([1, 1], [2, 6])],
      [pos([1, 1, 0], [2, 6, 11]), ['_Test\nTest_']]
    )
    testMatch(
      'explicit',
      '_Test\nTest_',
    ...
    testMatch(
      'explicit without multiline flag',
      '_Test\nTest_',
      [/_Test\nTest_/, pos([1, 1], [2, 6])],
      [pos([1, 1, 0], [2, 6, 11]), ['_Test\nTest_']]
    )
  ...
  testMatch(
    ...
    'Test',
    [/T(es(t))/, pos([1, 1], [1, 5])],
    [pos([1, 1, 0], [1, 5, 4]), ['Test', 'est', 't']]
  )
  testMatch('with non-match', 'Test', [/nope/, pos([1, 1], [1, 4])], undefined)

  testMatch(
    'un-anchored match in part of text',
    'Test',
    [/es/, pos([1, 1], [1, 4])],
    [pos([1, 2, 1], [1, 4, 3]), ['es']]
  )
  ...
  describe('with position restriction', () => {
    testMatch(
      'part of document, match anchored',
      'Test\nTest',
      [/^st\nTe$/, pos([1, 3], [2, 3])],
      [pos([1, 3, 2], [2, 3, 7]), ['st\nTe']]
    )
    testMatch(
      'part of document, match un-anchored',
      'Test\nTest',
      [/st\nTe/, pos([1, 3], [2, 3])],
      [pos([1, 3, 2], [2, 3, 7]), ['st\nTe']]
    )
    testMatch(
      'restriction prevents match',
      'Test\nTest',
      [/Test/, pos([1, 3], [2, 3])],
      undefined
    )
  ...
  testMatch(
    'match to end of file includes end of file',
    'test',
    (r) => [/test/, { start: point(1, 1), end: r.eof() }],
    (r) => [{ start: point(1, 1, 0), end: r.eof() }, ['test']]
  )

  testMatch(
    'match against empty string',
    '',
    [new RegExp(''), pos([1, 1], [1, 1])],
    undefined
  )
  testMatch(
    'empty match',
    'Test',
    [new RegExp(''), pos([1, 1], [1, 4])],
    [pos([1, 1, 0], [1, 1, 0]), ['']]
  )
  testMatch(
    'empty match with empty position restriction',
    'Test',
    [new RegExp(''), pos([1, 1, 0], [1, 1, 0])],
    undefined
  )
  testMatch(
    'empty match with position restriction',
    'Test',
    [new RegExp(''), pos([1, 1], [1, 2])],
    [pos([1, 1, 0], [1, 1, 0]), ['']]
  )
  testMatch(
    'if end is before start this is undefined',
    'Test\nTest',
    [/Test/, pos([2, 1], [1, 1])],
    undefined
  )
  testMatch(
    'if end is before start this is undefined (even with empty RegExp)',
    'Test\nTest',
    [new RegExp(''), pos([2, 1], [1, 1])],
    undefined
  )
...
describe('shift', () => {
  ...
  describe('out-of-bounds shifts', () => {
    testShift(
      'shifting beyond end of document',
      'test',
      [point(1, 1), 6],
      (r) => r.eof()
    )
    testShift(
      'shifting beyond end of document with newline',
      'test\n',
      [point(1, 1), 6],
      (r) => r.eof()
    )
    testShift(
      'shifting before start of document',
      'test',
      [point(1, 1), -1],
      point(1, 1, 0)
    )
  ...
  describe('in-bounds shifts', () => {
    ...
    testShift(
      'shift to start of next line (on newline)',
      'test\ntest',
      [point(1, 5), 1],
      point(2, 1, 5)
    )
    testShift(
      'shift to end of previous newline',
      'test\ntest',
      [point(2, 1), -1],
      point(1, 5, 4)
    )
    testShift(
      'shift to end of previous line',
      'test\ntest',
      [point(2, 1), -2],
      point(1, 4, 3)
    )
...
describe('distance', () => {
  ...
  describe('distance from a point to itself', () => {
    testDistance(
      'empty document, EOF',
      '',
      (r) => ({ start: r.eof(), end: r.eof() }),
      0
    )
    testDistance(
      'non-empty document, EOF',
      '',
      (r) => ({ start: r.eof(), end: r.eof() }),
      0
    )
  ...
  testDistance(
    'start before text and end after is just the BOF to EOF',
    'test1\ntest2',
    pos([-1, -3], [5, 7]),
    11
  )
