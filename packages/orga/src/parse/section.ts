...
export default (lexer: Lexer) =>
  <T extends Document | Section>(root: T): T => {
    ...
    const newSection = (props: { [key: string]: string } = {}): Section => {
      ...
      if (
        token &&
        token.type === 'drawer.begin' &&
        token.name.toLowerCase() === 'properties'
      ) {
    ...
    const parse = <T extends Document | Section | Footnote>(
      section: T,
